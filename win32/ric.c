...
size_t ric_encode(void **ptr,unsigned int width,unsigned int height,void *vdbitmap) {
  ...
  // Write description
  desc = (struct ric_description *)buffer;
  ...
  // Write sprite (bitmap)
  sprite = (struct ric_sprite *)(buffer+sizeof(struct ric_description));
  ...
  // Write Copy command
  copy = (struct ric_copybits *)(buffer+sizeof(struct ric_description)+sizeof(struct ric_sprite)+bitmap_size);
...
ssize_t ric_decode(void **ptr,void *data,size_t data_size,unsigned int *width,unsigned int *height) {
  ...
  while (i<data_size) {
    struct ric_opcode *op = (struct ric_opcode *)(((char *)data)+i);
  ...
  // Prepare bitmap
  bitmap = malloc(copybits->src.width*copybits->src.height);
